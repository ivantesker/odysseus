...
async def _auto_summarize_pass_single(days_back: int = 1, account_id: str | None = None, progress_cb=None) -> str:
    ...
    try:
        ...
        since = (datetime.now(UTC).replace(tzinfo=None) - _td(days=max(1, days_back))).strftime("%d-%b-%Y")
                        ...
                        if resp.ok:
                            ...
                            if summary:
                                ...
                                _c.execute("""
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                                """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, subject, sender, summary, model, datetime.now(UTC).replace(tzinfo=None).isoformat()))
                                _c.commit()
                                _c.close()
                    ...
                    try:
                        ...
                        if reply:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, reply, model, datetime.now(UTC).replace(tzinfo=None).isoformat()))
                            _c.commit()
                            _c.close()
                    ...
                    try:
                        ...
                        _cc.execute(
                            ...
                            (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid),
                             _cal_run_count, datetime.now(UTC).replace(tzinfo=None).isoformat())
                        )
                        _cc.commit()
                        _cc.close()
                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    outer_alert["Date"] = datetime.now(UTC).replace(tzinfo=None).strftime("%a, %d %b %Y %H:%M:%S +0000")
                        ...
                        if not resp.ok:
                            ...
                            if parsed is not None:
                                ...
                                """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), subject, sender,
                                      ...
                                      spam_reason, moved_to, model, datetime.now(UTC).replace(tzinfo=None).isoformat()))
                                _c.commit()
                                _c.close()
